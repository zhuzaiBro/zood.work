-- DeFi Staff 智能合约面试题（Prop AMM + DEX Aggregator + MEV）
...
VALUES (
  'f1a2b3c4-d5e6-4f78-9abc-def010000002',
  'Web3',
  'web3',
  2
)
...
VALUES (
  'f1a2b3c4-d5e6-4f78-9abc-def010000001',
  'DeFi Staff 智能合约面试题',
...
SELECT 'f1a2b3c4-d5e6-4f78-9abc-def010000001', id
...
WHERE id IN (
  'f1a2b3c4-d5e6-4f78-9abc-def010000101',
  'f1a2b3c4-d5e6-4f78-9abc-def010000102',
  'f1a2b3c4-d5e6-4f78-9abc-def010000103',
  'f1a2b3c4-d5e6-4f78-9abc-def010000104',
  'f1a2b3c4-d5e6-4f78-9abc-def010000105',
  'f1a2b3c4-d5e6-4f78-9abc-def010000106',
  'f1a2b3c4-d5e6-4f78-9abc-def010000107',
  'f1a2b3c4-d5e6-4f78-9abc-def010000108',
  'f1a2b3c4-d5e6-4f78-9abc-def010000109',
  'f1a2b3c4-d5e6-4f78-9abc-def010000110',
  'f1a2b3c4-d5e6-4f78-9abc-def010000111',
  'f1a2b3c4-d5e6-4f78-9abc-def010000112',
  'f1a2b3c4-d5e6-4f78-9abc-def010000113',
  'f1a2b3c4-d5e6-4f78-9abc-def010000114',
  'f1a2b3c4-d5e6-4f78-9abc-def010000115',
  'f1a2b3c4-d5e6-4f78-9abc-def010000116',
  'f1a2b3c4-d5e6-4f78-9abc-def010000117',
  'f1a2b3c4-d5e6-4f78-9abc-def010000118',
  'f1a2b3c4-d5e6-4f78-9abc-def010000119',
  'f1a2b3c4-d5e6-4f78-9abc-def010000120'
);
...
- 精简 `calldata` 结构，避免冗余字段$q1$,
  ...
  (
    'f1a2b3c4-d5e6-4f78-9abc-def010000102',
    'f1a2b3c4-d5e6-4f78-9abc-def010000001',
    '可升级合约的几种代理模式区别（Transparent vs UUPS vs Beacon）',
    $q2$## 板块
Solidity & EVM 基础（筛选关）
...
- 升级前 diff 布局 + 审计 + 存储布局测试$q3$,
  ...
  (
    'f1a2b3c4-d5e6-4f78-9abc-def010000104',
    'f1a2b3c4-d5e6-4f78-9abc-def010000001',
    'Foundry 的 Invariant Testing 是怎么工作的？给 Prop AMM 设计几个 invariant',
    $q4$## 板块
...
- 结合 **assume** 过滤不可达状态$q4$,
  ...
  (
    'f1a2b3c4-d5e6-4f78-9abc-def010000105',
    'f1a2b3c4-d5e6-4f78-9abc-def010000001',
    'Prop AMM 和传统 Uniswap V2/V3 AMM 的本质区别是什么？',
    $q5$## 板块
AMM 与做市机制（核心面）
...
## 备考提示
阅读 HumidiFi / Tessera 设计文档，能画「链下报价 → 链上验证 → 结算」时序图。$q5$,
    '困难',
    true,
    5
  ),
  (
    'f1a2b3c4-d5e6-4f78-9abc-def010000106',
    'f1a2b3c4-d5e6-4f78-9abc-def010000001',
    '什么是 LVR（Loss-Versus-Rebalancing），Prop AMM 怎么缓解它',
    $q6$## 板块
AMM 与做市机制（核心面）— **热点题**
...
- 提到 CoW Protocol batch auction、Uniswap X 等减少排序优势的方案$q6$,
    '困难',
    true,
    6
  ),
  (
    'f1a2b3c4-d5e6-4f78-9abc-def010000107',
    'f1a2b3c4-d5e6-4f78-9abc-def010000001',
    '设计一个 Vault 的流动性管理机制，需要考虑哪些边界情况？',
    $q7$## 板块
AMM 与做市机制（核心面）
...
## 案例引用
OpenZeppelin ERC4626 文档中的 **first depositor inflation attack** — 展示踩坑经验。$q7$,
    '困难',
    false,
    7
  ),
  (
    'f1a2b3c4-d5e6-4f78-9abc-def010000108',
    'f1a2b3c4-d5e6-4f78-9abc-def010000001',
    '集中流动性（Concentrated Liquidity）的核心数据结构是怎么设计的？',
    $q8$## 板块
AMM 与做市机制（核心面）
...
- 工程难点：跨 tick swap  gas 优化、flash accounting$q8$,
    '中等',
    false,
    8
  ),
  (
    'f1a2b3c4-d5e6-4f78-9abc-def010000109',
    'f1a2b3c4-d5e6-4f78-9abc-def010000001',
    '如何设计跨多个流动性来源（Uniswap/KyberSwap/1inch 等）的路由算法？',
    $q9$## 板块
...
## JD 关联
可提及 **Base Sequencer、交易排序、Private Order Flow**，体现对 infra 的了解。$q10$,
    '中等',
    false,
    10
  ),
...
- 动态 spread 在检测到 volatile mempool 时加宽$q12$,
    '中等',
    false,
    12
  ),
...
## 面试态度
不试图「禁止闪电贷」，而是让 **单 tx 内状态转移满足不变量**。$q14$,
    '困难',
    false,
    14
  ),
...
- CEI 模式 + `nonReentrant`
- Pull over push 付款
- 对外部 composability 假设 **view 也可能 lie during callback**$q15$,
    '困难',
    false,
    15
  ),
  (
    'f1a2b3c4-d5e6-4f78-9abc-def010000116',
    'f1a2b3c4-d5e6-4f78-9abc-def010000001',
    '协议 TVL 暴增 10 倍、交易量暴增 50 倍，合约/系统架构需要哪些改造？',
    $q16$## 板块
...
## Staff 加分
讲清 **哪些必须链上原子、哪些可链下 eventual consistency**。$q16$,
    '困难',
    true,
    16
  ),
...
## JD 关键词
「链下做市模型驱动链上参数调整」— 能画 sequence diagram。$q17$,
    '困难',
    true,
    17
  ),
  (
    'f1a2b3c4-d5e6-4f78-9abc-def010000118',
    'f1a2b3c4-d5e6-4f78-9abc-def010000001',
    'Solana 上做 Prop AMM 和 EVM 上的核心架构差异是什么？',
    $q18$## 板块
Solana 相关（加分项）
...
## Prop AMM 含义
Solana 上可把 **每个 market maker 报价** 拆成独立 account，链上撮合读多账户 — 天然适合 prop model。$q18$,
    '中等',
    false,
    18
  ),
...
-- 导入后访问: /interview/f1a2b3c4-d5e6-4f78-9abc-def010000001
