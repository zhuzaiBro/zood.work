...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const { chapterId, orderedLessonIds } = body as {
      chapterId?: string;
      orderedLessonIds?: string[];
    };

    if (!chapterId || !Array.isArray(orderedLessonIds) || orderedLessonIds.length === 0) {
      return NextResponse.json({ error: '缺少章节 ID 或课时排序列表' }, { status: 400 });
    }
    ...
    if (fetchError) {
      console.error('查询课时失败:', fetchError);
    ...
    if (uniqueIds.length !== existingIds.size) {
      return NextResponse.json({ error: '课时列表与章节不匹配' }, { status: 400 });
    }

    for (const id of uniqueIds) {
      if (!existingIds.has(id)) {
        return NextResponse.json({ error: '包含不属于该章节的课时' }, { status: 400 });
      }
    }
    ...
    if (failed?.error) {
      console.error('更新课时排序失败:', failed.error);
      return NextResponse.json({ error: failed.error.message }, { status: 500 });
    }

    return NextResponse.json({ success: true });
  } catch (error: unknown) {
    console.error('课时排序异常:', error);
