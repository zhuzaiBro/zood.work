...
export async function GET() {
  try {
    ...
    const message = error instanceof Error ? error.message : '加载购买意向失败';
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (!Number.isInteger(id) || id <= 0) {
      return NextResponse.json({ error: '无效的购买意向 ID' }, { status: 400 });
    }
    ...
    const message = error instanceof Error ? error.message : '更新购买意向失败';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}
